...
The function trained several different models with seven different possible random subset sizes. The highest area under the ROC curve, `0.8247820`, was achieved when only 2 randomly chosed predictors were considered.
...
The plot lists the importance of any given variable to the the model, with the most important variable at the top and least important variable at the bottom.(4) Since there is only a single jump after `plasmaGlucose`, all variable appear to be significant to the model. We conclude that it is not necessary to eliminate variables and retrain the model.
